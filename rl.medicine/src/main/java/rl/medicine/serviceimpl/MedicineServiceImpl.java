...
public class MedicineServiceImpl implements MedicineService{
	...
	/**
	 * Get Medicine By Expiry Date
	 */
	public HashMap<Date, MedicineDetailDTO> getMedicineByExpiryDate(String selectedMedicineName) {
		final String METHOD_TAG = TAG+" getMedicineByExpiryDate";
		appDebug("Inside the "+METHOD_TAG);
		final List<MedicineDetailDTO> medicineInformationByExpiryDateList = appDao.getMedicineExpiryDateList(selectedMedicineName);
		if(medicineInformationByExpiryDateList != null && !medicineInformationByExpiryDateList.isEmpty()) {
			final HashMap<Date, MedicineDetailDTO> medicineListByExpiryDate = new HashMap<>();
			for(int index = 0; index < medicineInformationByExpiryDateList.size(); index++) {
				final MedicineDetailDTO listData = medicineInformationByExpiryDateList.get(index);
				
				MedicineDetailDTO dataToHashMap = null;
				if(medicineListByExpiryDate.containsKey(listData.getMedicineExpiryDate())) {
					dataToHashMap = medicineListByExpiryDate.get(listData.getMedicineExpiryDate());
				}else {
					dataToHashMap = new MedicineDetailDTO();
				}
				
				final String medicineName = listData.getMedicineName();
				dataToHashMap.setMedicineName(medicineName);
				dataToHashMap.setMedicineNameId(listData.getMedicineNameId());
				final String boxName = dataToHashMap.getMedicineBox();
				if(boxName == null || boxName.isEmpty()) {
					dataToHashMap.setMedicineBox(listData.getMedicineBox());
				}else {
					dataToHashMap.setMedicineBox(boxName+", "+listData.getMedicineBox());
				}
				final int strips = listData.getStrip();
				final int leftQuantity = listData.getLeftQuantity();
				
				appDebug(METHOD_TAG+"  Strip from list database : "+strips+" left quantity : "+leftQuantity+" for expiry date "+listData.getMedicineExpiryDate());
				dataToHashMap.setMedicineBoxId(listData.getMedicineBoxId());
				dataToHashMap.setLeftQuantity(dataToHashMap.getLeftQuantity()+leftQuantity);
				dataToHashMap.setStrip(dataToHashMap.getStrip()+strips);
				dataToHashMap.setMedicineExpiryDate(listData.getMedicineExpiryDate());
				
				int totalQuantity = 0; 
				if(medicineName.contains("15")) {
					appDebug(METHOD_TAG+" Medicine Name "+medicineName+" is for 15.");
					totalQuantity = strips * 15;
				}else if(medicineName.contains("12")) {
					appDebug(METHOD_TAG+" Medicine Name "+medicineName+" is for 12.");
					totalQuantity = strips * 12;
				}else if(medicineName.contains("6")) {
					appDebug(METHOD_TAG+" Medicine Name "+medicineName+" is for 6.");
					totalQuantity = strips * 6;
				}else {
					appDebug(METHOD_TAG+" Medicine Name "+medicineName+" is for 10.");
					totalQuantity = strips * 10;
				}
				totalQuantity = totalQuantity + leftQuantity;
				appDebug(METHOD_TAG+" from hash object total quantity : "+dataToHashMap.getTotalQuantity());
				totalQuantity = dataToHashMap.getTotalQuantity() + totalQuantity;
				appDebug(METHOD_TAG+" total quantity : "+totalQuantity);
				dataToHashMap.setTotalQuantity(totalQuantity);

				appDebug(METHOD_TAG+" data on hash map : "+dataToHashMap);
				
				medicineListByExpiryDate.put(listData.getMedicineExpiryDate(), dataToHashMap);
			}
			return medicineListByExpiryDate;
		}
		return null;
	}
